...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
            ...
            if (entity instanceof EntityLivingBase &&
                ...
                if (watchableObject.getDataValueId() == 6 && watchableObject.getObjectType() == 3) {
                    ...
                    if (dif >= 0.0F && newHealth != 10.0F)
                        RadiumClient.getInstance().getEventBus().post(new EntityHealthUpdateEvent(elb, dif));
                }
            }
    ...
    public void handleAnimation(S0BPacketAnimation packetIn) {
        ...
        if (entity != null) {
            if (packetIn.getAnimationType() == 0) {
                ...
                RadiumClient.getInstance().getEventBus().post(new EntitySwingEvent(entitylivingbase));
                ...
                spawnParticleEntityEvent(entity, 0);
            } else if (packetIn.getAnimationType() == 5) {
                spawnParticleEntityEvent(entity, 1);
            }
        }
    }
    ...
    public void handleTitle(S45PacketTitle packetIn) {
        ...
        if (!event.isCancelled())
            this.gameController.ingameGUI.displayTitle(s, s1,
                                                       packetIn.getFadeInTime(), packetIn.getDisplayTime(), packetIn.getFadeOutTime());
    }
    ...
    private void spawnParticleEntityEvent(Entity entity, int type) {
        final EnumParticleTypes particleType = type == 0 ? EnumParticleTypes.CRIT : EnumParticleTypes.CRIT_MAGIC;
        final int multiplier;
        if (entity instanceof EntityPlayerSP) {
            multiplier = 1;
        } else {
            final SpawnParticleEntityEvent event = new SpawnParticleEntityEvent(entity, type, 1);
            RadiumClient.getInstance().getEventBus().post(event);
